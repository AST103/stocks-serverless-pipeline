...
resource "aws_cloudwatch_event_rule" "stock_check" {
  ...
  schedule_expression = "cron(30 8 ? * TUE-SAT *)" # changed to 8:30am UTC which is 12:30am PST, so it should run after market close and give the API enough time to update with the latest data (since the API updates around 8am UTC)
}
